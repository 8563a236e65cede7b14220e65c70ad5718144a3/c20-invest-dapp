...
C20Invest and C20InvestProxy. This causes C20InvestProxy to error
out if run in the same session as C20Invest. Thus this single test
is run in isolation.
...
They will be located inside the run director within the root
...
The :ref:`Main` section contains the C20Invest contract and its proxy. The
...
The :ref:`Proxy Contracts` section contains the OpenZeppelin proxy contract implementations,
...
The :ref:`Access` section contains a custom Ownable contract that takes a
...
The :ref:`Math` section contains a newer version of the SafeMath library
than the version in "C20 Base Contracts/SafeMath" and is accompanied
by a syntax change.

The :ref:`Utils` section contains a custom Suspendable contract which has
